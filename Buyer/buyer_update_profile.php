...
class UserProfile {
    public function getProfileDetails($conn, $user_id) {
        $stmt = $conn->prepare("SELECT p.*, u.phone_num, u.email FROM profile p JOIN users u ON p.user_id = u.user_id WHERE p.user_id = ?");
    ...
    public function updateProfile($conn, $data) {
        ...
        if ($stmt->affected_rows === 0) {
            error_log("No rows were updated. User ID: " . $data['user_id']);
        }
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="button-group">
                    ...
                    <a href="buyer_dashboard.php">
                        <button type="button" class="secondary-button">Return to Dashboard</button>
                    </a>
                </div>
            </form>
        </div>
